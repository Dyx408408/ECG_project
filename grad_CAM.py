...
def plot_gradcam(ecg_signal, cam, idx, channel=0):
    ...
    # plt.legend()
    # plt.tight_layout()
    if not os.path.exists(os.path.join('dyx/grad_cam1', str(idx))):
        os.mkdir(os.path.join('dyx/grad_cam1', str(idx)))
    plt.savefig(os.path.join('dyx/grad_cam1', str(idx), str(channel)))

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data-dir', type=str, default='dyx/CPSC', help='Directory for data dir')
    ...
    parser.add_argument('--model-path', type=str, default='dyx/checkpoint/best_model.pth', help='Path to saved model')
...
if __name__ == '__main__':
    ...
    model.eval()  # 设置为评估模式
    model.load_state_dict(torch.load('./dyx/models/best_model.pth', map_location=device))
    ...
    # idx = 'A5373'
    # 读取对应的ECG数据
    ecg_data = pc.load_challenge_data(f"dyx/CPSC/{idx}.mat")
